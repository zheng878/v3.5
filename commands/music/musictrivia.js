...
module.exports = class MusicTriviaCommand extends Command {
  ...
  async run(message) {
    ...
    const infoEmbed = new MessageEmbed()
      ...
      .setTitle('開始音樂小測試')
      .setDescription(
        `請準備! 這裡有 ${numOfLinks} 首歌曲, 你有 45 秒的時間來猜歌曲的演唱者/樂團 和 歌曲名稱. 祝您順利!`
      );
          ...
          collector.on('collect', m => {
            ...
            // if user guessed singer
              ...
              m.content.toLowerCase() ===
                queue[0].singer.toLowerCase() + ' ' + queue[0].title.toLowerCase() ||
              m.content.toLowerCase() === queue[0].title.toLowerCase() + ' ' + queue[0].singer.toLowerCase()
            ) {
              if (
        ...
        .on('end', () => {
          if (queue.length >= 1) {
            ...
            if (highestTriviaScore == 0 || isHighestValueDuplicate)
              return message.channel.send('沒有人獲勝,祝您們下次好運!');
            message.channel.send(
              `贏家是 ${winner}, 他有 ${highestTriviaScore} 分`
            );
            message.guild.musicData.isPlaying = false;
            message.guild.triviaData.isTriviaRunning = false;
            message.guild.triviaData.triviaScore.clear();
